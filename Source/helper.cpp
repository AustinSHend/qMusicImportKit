...
QVariant safeRefCellValue(const QXlsx::Document *workbook, int row, int col) {
    QXlsx::Cell *cell = workbook->cellAt(row, col);
    if(!cell) {
        return "";
    }
    else {
...
QXlsx::Format safeRefCellFormat(const QXlsx::Document *workbook, int row, int col) {
    QXlsx::Cell *cell = workbook->cellAt(row, col);
    if(!cell) {
        return QXlsx::Format();
    }
...
// Inserts user data into a workbook and sorts all data
// Also dedupes data based on the first cell
bool insertInWorkbook (QString lastFolder, QString logScore, QString notes) {
    QSettings MIKSettings;

    // If the workbook from the settings doesn't exist, return
    if(!QFileInfo(MIKSettings.value("sDefaultXLSXSheetLocation", "").toString()).isFile()) {
        return false;
    }

    // Initialize workbook from settings and its first worksheet
    QXlsx::Document workbook(MIKSettings.value("sDefaultXLSXSheetLocation", "").toString());

    // Dummy formatting
    QXlsx::Format textFormat;
    textFormat.setNumberFormat("@");

    // Write user data to the end of the spreadsheet
    // After the first write, the rowCount is updated and no longer needs the +1
    workbook.write(workbook.dimension().rowCount()+1, 1, lastFolder, textFormat);
    workbook.write(workbook.dimension().rowCount()  , 2, logScore, textFormat);
    workbook.write(workbook.dimension().rowCount()  , 3, notes, textFormat);

    // Map to hold each row as a single unit
    // Maps are inherently sorted as values are added to them
    QMap<QString, QStringList> rowMap;

    // Insert all spreadsheet data into a map, deduping on the way
    // Iterate through each row (i = 2 skips first row (header))
    for(int i = 2; i <= workbook.dimension().rowCount(); i++) {
        // If cell 1 of the column (key value) is empty, ignore the row entirely
        // Ignored rows will be deleted later
        if(safeRefCellValue(&workbook, i, 1).toString() != "") {
            // Temporary QStringList to hold columns 2 -> infinity, for later insertion into the map
            QStringList tempRow;
            // Iterate through each column (j = 2 skips first column (key column))
            for(int j = 2; j <= workbook.dimension().columnCount(); j++) {
                // Add the current cell to the tempRow
                tempRow << (safeRefCellValue(&workbook, i, j).toString());
            }
            // Insert the tempRow into the map based on the first column's cell (key value)
            rowMap.insert(safeRefCellValue(&workbook, i, 1).toString(), tempRow);
        }
    }

    // Use a const_iterator to traverse the map and dump all values back into the spreadsheet
    QMap<QString, QStringList>::const_iterator it = rowMap.constBegin();
    int row = 2;
    while(it != rowMap.constEnd()) {
        // Insert the key into the first cell
        workbook.write(row, 1, it.key());

        // Insert every other cell into the next columns
        for(int j = 0; j < it.value().size(); j++) {
            workbook.write(row, j+2, it.value()[j]);
        }
        row++;
        it++;
    }

    // Remove all remaining rows that exist
    for(int i = row; i <= workbook.dimension().rowCount(); i++) {
        for(int j = 1; j <= workbook.dimension().rowCount(); j++) {
            workbook.write(i, j, "");
        }
    }
